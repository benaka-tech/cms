...
def t_dashboard(request):
    if request.user.is_teacher:
        return render(request, 'info/t_dashboard.html')

@login_required
def dashboard(request, stud_id, class_id):
    if request.user.is_teacher:
        return render(request, 'info/t_dashboard.html')
    if request.user.is_student:
        col = ['bg-danger', 'bg-warning', '', 'bg-info']
        ...
        for ass in ass_list:
            try:
                ...
                cou = StudentCourse.objects.get(student=stud, course=ass.course)

                course.append(cou.course.name)
            ...
            sc_list.append(sc)
        for sc in sc_list:
            for m in sc.marks_set.filter(name__contains='Internal test 1'):
                marks.append(m.marks1)

        for sc in sc_list:
            for m in sc.marks_set.filter(name__contains='Internal test 2'):
                marks1.append(m.marks1)
                print(marks1)

        for sc in sc_list:
            for m in sc.marks_set.filter(name__contains='Internal test 3'):
                marks2.append(m.marks1)

        return render(request, 'info/dashboard.html',
                      {'att_list': att_list, 'col': col, 'matrix': matrix, 'sc_list': course, 'marks': marks,
